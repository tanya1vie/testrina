...
const svg = d3
  .select("body") // Select the body or any other container where you want the SVG to be appended
  .append("svg")
...
d3.csv("./data-files/data_fp4.csv").then(function (data) {
  ...
  svgScatterplot.append("g").call(yAxis);

  // Add a text element for labels
  const label = svgScatterplot
    .append("text")
    .attr("class", "label")
    .attr("x", 0)
    .attr("y", 0)
