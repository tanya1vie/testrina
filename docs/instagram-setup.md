...
This version replaces the hand-written project cards with posts from the professional Creator account `@mani__festations`. GitHub Actions refreshes `shared/data/instagram-posts.json` once per day; the browser never receives the private access token.
