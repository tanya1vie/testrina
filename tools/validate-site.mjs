...
const legacyWixRouteMap = [
  ["about", "about.html"],
  ["bach", "bach.html"],
  ["berryblitz", "berryblitz.html"],
  ["bookstore", "bookstore.html"],
  ["bruce", "bruce.html"],
  ["cave", "cave.html"],
  ["conservatory", "conservatory.html"],
  ["copy-2-of-template", "copy-2-of-template.html"],
  ["copy-2-of-template-1", "copy-2-of-template-1.html"],
  ["copy-3-of-template", "copy-3-of-template.html"],
  ["copy-of-core-ii-studio", "copy-of-core-ii-studio.html"],
  ["copy-of-hostile-pantry", "copy-of-hostile-pantry.html"],
  ["copy-of-projects", "copy-of-projects.html"],
  ["copy-of-spatial", "copy-of-spatial.html"],
  ["copy-of-template", "copy-of-template.html"],
  ["crab", "crab.html"],
  ["deeptime", "deeptime.html"],
  ["feltmate", "feltmate.html"],
  ["figure", "figure.html"],
  ["flatland", "flatland.html"],
  ["gallery", "gallery.html"],
  ["hostilesalad", "hostilesalad.html"],
  ["houseofcards", "houseofcards.html"],
  ["intothewoods", "intothewoods.html"],
  ["janus", "janus.html"],
  ["maladies", "maladies.html"],
  ["marche", "marche.html"],
  ["matter", "matter.html"],
  ["microhouse", "microhouse.html"],
  ["ocad1", "ocad1.html"],
  ["ocad2", "ocad2.html"],
  ["ossurary", "ossurary.html"],
  ["pheno", "pheno.html"],
  ["pier", "pier.html"],
  ["polysapien", "polysapien.html"],
  ["portfoliobook", "portfoliobook/index.html"],
  ["preschool", "preschool.html"],
  ["projects-1", "projects-1.html"],
  ["projections", "projections.html"],
  ["sanfran", "sanfran.html"],
  ["seaweed", "seaweed.html"],
  ["seesaw", "seesaw.html"],
  ["stratum", "stratum.html"],
  ["swrm", "swrm.html"],
  ["tort", "tort.html"],
...
async function checkReference(source, reference) {
  ...
  if (await exists(normalizedTarget)) {
    if (!(await existsWithExactCase(normalizedTarget))) {
      failures.push(
        `${relative(root, source)}: local reference has incorrect capitalization: ${reference}`,
      );
      return;
    }
    ...
    if (
      (await exists(join(normalizedTarget, "index.html"))) &&
      (await existsWithExactCase(join(normalizedTarget, "index.html")))
    )
      return;
  }
  if (
...
async function validateHtml(path) {
  ...
  for (const image of accessibilityHtml.matchAll(/<img\b[^>]*>/gi)) {
    if (!/\balt\s*=\s*["'][^"']*["']/i.test(image[0])) {
      failures.push(
        `${relative(root, path)}: image is missing an alt attribute: ${image[0].slice(0, 100)}`,
      );
    }
  }
  const references = [
...
for (const [route, target] of legacyWixRouteMap) {
  if (!(await exists(join(root, target))))
    failures.push(`Wix route /${route} has no migration target: ${target}`);
}
...
for (const path of nativeMigratedPages) {
  ...
  const requiredMarkers = [
    ...
    'href="assets/css/projectPage.css"',
    'src="assets/js/siteChrome.js"',
  ];
  for (const marker of requiredMarkers) {
    if (!html.includes(marker))
      failures.push(`${path}: migrated-page style/accessibility marker is missing: ${marker}`);
  }

  for (const image of html.matchAll(/<img\b[^>]*>/gi)) {
    if (!/\balt\s*=\s*["'][^"']+["']/i.test(image[0])) {
      failures.push(
        `${path}: migrated image needs non-empty alternative text: ${image[0].slice(0, 100)}`,
      );
    }
  }
...
const retiredReferences = ["CSS Files/", "JavaScript Files/", "scripts/sync-instagram.mjs"];
...
for (const path of [...browserScripts, ...maintenanceScripts]) {
  ...
  if (result.status !== 0) {
    failures.push(
      `${relative(root, path)}: JavaScript syntax check failed\n${result.stderr.trim()}`,
    );
  }
}
...
const accessibilityContract = [
  ...
  [
    homePage,
    'aria-label="Explore work by discipline"',
    "Work explorer navigation label is missing",
  ],
