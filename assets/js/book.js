document.addEventListener('DOMContentLoaded', () => {
  ...
  function addMagazineBookStyles() {
    ...
    style.textContent = `
      .thesis-book-archive { width: 100%; margin: 72px 0; }
      .thesis-book-archive h2 { margin: 0 0 24px; text-align: center; font-size: 14px; font-weight: 400; letter-spacing: 0.18em; text-transform: uppercase; }
      .magazine-booklet-wrap { width: min(82vw, 980px); margin: 0 auto; position: relative; padding: 0 44px; box-sizing: border-box; }
      .magazine-booklet { width: 100%; aspect-ratio: 1.48 / 1; display: flex; position: relative; overflow: visible; perspective: 1800px; filter: drop-shadow(0 10px 18px rgba(0,0,0,0.16)); }
      .magazine-page { position: relative; flex: 1 1 50%; min-width: 0; background-color: #fffdf7; background-position: center; background-repeat: no-repeat; background-size: contain; backface-visibility: hidden; transform-style: preserve-3d; }
      .magazine-page.verso { border-right: 1px solid rgba(0,0,0,0.12); transform-origin: right center; }
      .magazine-page.recto { border-left: 1px solid rgba(0,0,0,0.12); transform-origin: left center; }
      .magazine-booklet.is-cover .magazine-page.verso { opacity: 0; pointer-events: none; }
      .magazine-booklet.is-cover::after { display: none; }
      .magazine-booklet::after { content: ''; position: absolute; inset: 0 auto 0 50%; width: 1px; background: rgba(0,0,0,0.08); box-shadow: 0 0 10px rgba(0,0,0,0.12); pointer-events: none; z-index: 5; }
      .magazine-page.flip-forward { animation: magazineFlipForward 720ms cubic-bezier(.25,.8,.25,1); z-index: 4; }
      .magazine-page.flip-backward { animation: magazineFlipBackward 720ms cubic-bezier(.25,.8,.25,1); z-index: 4; }
      @keyframes magazineFlipForward { from { transform: rotateY(0deg); } to { transform: rotateY(-180deg); } }
      @keyframes magazineFlipBackward { from { transform: rotateY(0deg); } to { transform: rotateY(180deg); } }
      .magazine-booklet-arrow { position: absolute; top: 50%; transform: translateY(-50%); border: 0; background: transparent; color: inherit; padding: 8px; font: inherit; font-size: clamp(28px, 4vw, 42px); line-height: 1; cursor: pointer; z-index: 10; }
      .magazine-booklet-arrow.prev { left: 0; }
      .magazine-booklet-arrow.next { right: 0; }
      .magazine-booklet-arrow:disabled { opacity: 0.2; cursor: default; }
      .magazine-booklet-status { margin: 12px 0 0; text-align: center; font-size: 0.88rem; opacity: 0.65; }
      @media (max-width: 700px) { .thesis-book-archive { margin: 52px 0; } .magazine-booklet-wrap { width: 100%; padding: 0 32px; } .magazine-booklet { aspect-ratio: 1.36 / 1; } .magazine-booklet-arrow { font-size: 30px; } }
      @media (prefers-reduced-motion: reduce) { .magazine-page.flip-forward, .magazine-page.flip-backward { animation-duration: 1ms; } }
    `;
